...
function delete_convert(id,type) {
  "use strict";
    if (confirm("Are you sure?")) {
      var url = site_url + 'accounting/delete_convert/'+id+'/'+type;
...
function init_sales_table() {
  ...
  $('#payment-table').appTable({
    source: site_url + 'accounting/sales_table',
            filterDropdown: [
            {name: "group_id", class: "w200", options: []},
            {name: "quick_filter", class: "w200", options: []}
            ],
            ...
            columns: [
              ...
              {title: "Invoice"},
              {title: "Status"},
              {title: 'Convert', "class": "text-center w100", visible: showOptions}
            ],
...
function init_sales_invoice_table() {
  ...
  initDataTable('.table-sales-invoice', site_url + 'accounting/sales_invoice_table', [0], [0], fnServerParams, [1, 'desc']);
}
...
function bulk_action(event) {
  "use strict";
    if (confirm_delete()) {
        ...
        setTimeout(function() {
            $.post(site_url + 'accounting/transaction_bulk_action', data).done(function() {
