...
(function($) {
    ...
    dashboard_custom_view('last_30_days',"<?php echo app_lang('last_30_days'); ?>",'last_30_days');
})(jQuery);
...
function dashboard_custom_view(value, $lang, custom_input_name, clear_other_filters) {
    ...
    requestGet('accounting/get_data_dashboard?date_filter=' + value).done(function(response) {
        ...
        const chart = Highcharts.chart('profit_and_loss', {
            ...
            title: {
                text: '<?php echo app_lang('profit_and_loss'); ?>'
            },
            ...
            yAxis: {
                title: {
                    text: '<?php echo html_entity_decode($currency); ?>'
                }
            },
            xAxis: {
                categories: ['<?php echo app_lang('acc_net_income'); ?>', '<?php echo app_lang('acc_income'); ?>', '<?php echo app_lang('expenses'); ?>']
            },
            credits: {
              enabled: false
            },
    ...
    Highcharts.chart('expenses_chart', {
        ...
        title: {
            text: '<?php echo app_lang("expense_account_overview"); ?>'
        },
        credits: {
          enabled: false
        },
        tooltip: {
            pointFormat: '<b>{point.amount} <?php echo html_entity_decode($currency); ?></b>'
        },
        ...
        plotOptions: {
            pie: {
                dataLabels: {
                    ...
                    formatter: function() {
                        console.log(this);
                        return '<b>'+this.key+'</b>: '+Highcharts.numberFormat(this.point.amount,2)+ ' <?php echo html_entity_decode($currency); ?>';
                    }
                },
    ...
    Highcharts.chart('sales_chart', {
        ...
        title: {
            text: '<?php echo app_lang("cash_flow"); ?>'
        },
    ...
    Highcharts.chart('income_chart', {
        ...
        title: {
            text: '<?php echo app_lang("acc_income"); ?>'
        },
        credits: {
              enabled: false
            },
