...
class Candidate_signin extends App_Controller {
    ...
    function index() {
        if ($this->candidates_model->login_user_id()) {
            ...
            return $this->template->view('Recruitment\Views\recruitment_portal\signin/index', $view_data);
        }
    }
    ...
    function authenticate() {
        ...
        if (!$this->candidates_model->authenticate($email, $password)) {
    ...
    function sign_out() {
        $this->candidates_model->sign_out();
    }
    ...
    function send_reset_password_mail() {
        ...
        $existing_user = $this->candidates_model->is_email_exists($email);
        ...
        if ($existing_user) {
            ...
            $parser_data["ACCOUNT_HOLDER_NAME"] = $existing_user->candidate_name . " " . $existing_user->last_name;
            ...
            $parser_data['RESET_PASSWORD_URL'] = get_uri("candidate_signin/new_password/" . $verification_info->code);
    ...
    function do_reset_password() {
        ...
        if ($valid_key) {
            $email = get_array_value($valid_key, "email");
            $user = $this->candidates_model->is_email_exists($email);
            $user_data = array("password" => password_hash($password, PASSWORD_DEFAULT));
            if ($user->id && $this->candidates_model->ci_save($user_data, $user->id)) {
                ...
                echo json_encode(array("success" => true, 'message' => app_lang("password_reset_successfully") . " " . anchor("candidate_signin", app_lang("signin"))));
                return true;
            }
        }
        echo json_encode(array("success" => false, 'message' => app_lang("error_occurred")));
