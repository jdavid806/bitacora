...
fbOptions.typeUserEvents = {
	...
	'table': {
		onadd: function(fId) {
			do_form_field_restrictions(fId, 'table');
		},
	},
}
$(function() {
...
$(function(){
	...
	$.loadScript('//cdnjs.cloudflare.com/ajax/libs/require.js/2.3.2/require.min.js', function(){
		require.config({
			baseUrl : 'js',
			paths : {
				'jquery-ui': '//cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.min',
				'form-builder' : site_url+'/plugins/Recruitment/assets/plugins/form-builder/form-builder.min'
			},
	   waitSeconds : 10 // optional
		...
		var start = function() {
			...
			$('body').on('click','#sm_btn2',function() {
				...
				$('.recruitment-channel-add-edit').on('submit');
	...
	$('body').on('blur', '.form-field.editing', function () {
		AppHelper.settings.disableKeyboardShortcuts = 1;
	});
