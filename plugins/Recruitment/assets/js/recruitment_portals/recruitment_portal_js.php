<script type="text/javascript">
	function show_more_job(argument) {
		...
		if(!disbled_button){
			...
			$.post("<?php echo get_uri("recruitment_portal/show_more_job") ?>", data).done(function(response) {
...
$(".kb-search-input").keyup(function(){
	...
	$.post("<?php echo get_uri("recruitment_portal/job_live_search") ?>", data).done(function(response) {
		response = JSON.parse(response); 
		if(response.status == 'true' || response.status == true){
			$('#panel_body_job').html(''); 
			$('#panel_body_job').append(response.data); 
