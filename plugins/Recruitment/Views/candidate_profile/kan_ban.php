            ...
            <ul class="status tasks-status sortable relative" data-task-status-id="<?php echo html_entity_decode($status['id']); ?>">
              ...
              foreach ($candidates as $candidate) {
                ...
                <li class="text-center not-sortable kanban-load-more" data-load-status="<?php echo html_entity_decode($status['id']); ?>">
                 ...
                 <a href="#" class="btn btn-default btn-block<?php if($total_pages <= 1){echo ' disabled';} ?>" data-page="1" onclick="candidate_kanban_load_more(<?php echo html_entity_decode($status['id']); ?>,this,'recruitment/recruitment_kanban_load_more',265,360); return false;";><?php echo app_lang('load_more'); ?></a>
               </li>
               <?php } ?>
               <li class="text-center not-sortable mtop30 kanban-empty<?php if($total_candidates > 0){echo ' hide';} ?>">
                <h4>
                  ...
                  <?php echo app_lang('no_candidate_found'); ?></h4>
                </li>
              </ul>
            </div>
          </div>
