...
class Outlook_smtp {
    ...
    public function __construct($mailbox_info = null) {
        ...
        $this->redirect_uri = get_uri("mailbox_microsoft_api/save_outlook_smtp_access_token");
    ...
    public function get_authorization_url($mailbox_id = 0) {
        ...
        $auth_array = array(
            "client_id" => $mailbox_info->outlook_smtp_client_id,
            "response_type" => "code",
    ...
    public function save_access_token($mailbox_id = 0, $code = "", $is_refresh_token = false) {
        ...
        $fields = array(
            "client_id" => $mailbox_info->outlook_smtp_client_id,
            "client_secret" => $mailbox_info->outlook_smtp_client_secret,
            "redirect_uri" => $this->redirect_uri . "/$mailbox_id",
            "scope" => "Mail.Send",
        ...
        if ($new_access_token) {
            $data = array("outlook_smtp_oauth_access_token" => $new_access_token);
            ...
            $data = array("outlook_smtp_authorized" => "1");
            $this->Mailboxes_model->ci_save($data, $mailbox_id);

            //send test email if any
            $test_mail_to = get_mailbox_setting("send_test_mail_to_" . $mailbox_id);
            ...
            if ($test_mail_to && !$is_refresh_token) {
                ...
                $this->Mailbox_settings_model->save_setting("send_test_mail_to_" . $mailbox_id, "");
            }
        }
    }
    ...
    private function do_request($method, $path = "", $body = array(), $decode_result = true) {
        ...
        $oauth_access_token = json_decode($this->mailbox_info->outlook_smtp_oauth_access_token);
