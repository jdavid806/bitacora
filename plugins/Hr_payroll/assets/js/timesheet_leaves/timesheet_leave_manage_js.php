<script>
	...
	function attendance_filter(invoker){
		...
		$.post("<?php echo get_uri("hr_payroll/timesheets_leave_filter") ?>", data).done(function(response) {
	...
	function timesheet_leave_calculation(event) {
		...
		var data = {};
		data.month = $("#month_attendance").val();
		$.post("<?php echo get_uri("hr_payroll/timesheet_leave_calculation") ?>", data).done(function(response) {
			response = JSON.parse(response);
