<script>
	...
	function daily_synchronized(event){
		'use strict';
		...
		if(valid_employees_value){
			...
			$('input[name="hrp_attendance_value"]').val(JSON.stringify(commodity_fill.getData()));   
			$('input[name="attendance_fill_month"]').val($("#month_attendance").val());
			$('input[name="hrp_attendance_rel_type"]').val('daily_synchronization');   
			$( "#add_attendance" ).submit();
		}
	...
	$('#month_attendance').on('change', function() {
		...
		$('.timesheet_leaves_url').attr('href', '<?php echo get_uri("hr_payroll/manage_attendance_timesheet_leaves/") ?>'+$('#month_attendance').val());
		attendance_filter();
