 ...
 */
	...
	public function synchronization_attendance($data)
	{
		...
		$builder = $this->db->table(get_db_prefix().'hrp_employees_timeshee_leaves');
		$builder->select($str_select_day);
		$builder->where('rel_type', $rel_type);
		$builder->where("date_format(month, '%Y-%m-%d') = '".$attendance_month."'");
		$builder->orderBy('staff_id', 'asc');
		$employees_timesheet_leaves = $builder->get()->getResultArray();
		if(count($employees_timesheet_leaves) > 0){
			...
			if(count($insert_batch_employees_timesheet) > 0){
				$builder->updateBatch(get_db_prefix().'hrp_employees_timeshee_leaves', $insert_batch_employees_timesheet, 'id');
			}
		}else{
			...
			$builder->insertBatch(get_db_prefix().'hrp_employees_timeshee_leaves', $staff_timesheet_leave_data);
		}
				...
				// deduction_: salary deduction
					...
					if($payroll_formular != '0'){
						if(isset($taxable_salary_index) &&  $payroll_key == 'income_tax_paye'){
							$f = str_replace(($number_to_anphabe[$taxable_salary_index].'6'), $number_to_anphabe[$taxable_salary_index].($staff_row+1), $payroll_formular);
						}else{
		...
		foreach ($payslip_detail as $key => $value) {
			...
			foreach ($value as $payroll_key => $payroll_value) {
				if($payroll_key == 'payment_run_date'){
					...
					$payslip_detail[$key][$payroll_key] = to_sql_date1($payroll_value);
				}
	...
	{	
		...
		$days_header_in_month = $this->get_day_header_in_month($attendance_month, '', false);
		$header_key = array_merge($days_header_in_month['staff_key'], $days_header_in_month['days_key'], $days_header_in_month['attendance_key']);
	...
	public function timesheet_leave_calculation($data)
	{
		...
		if(count($update_batch_data) > 0){
			$builder = $this->db->table(get_db_prefix().'hrp_employees_timeshee_leaves');
			$builder->updateBatch($update_batch_data, 'id');
			...
			if(count($employees_timesheets) > 0){
				...
				$builder->insertBatch($update_batch_employees_timesheet);
			}
	...
	public function attendance_calculation_v2($payslip_month, $arr_payslip_range, $where = '')
	{
		...
		$builder = $this->db->table(get_db_prefix().'hrp_employees_timeshee_leaves');
		$builder->select($str_select_temp.', id, staff_id, month');
		if($where != ''){
			$builder->where($where);
		}
		$builder->where('rel_type', $rel_type);
	...
	{
		$builder = $this->db->table(get_db_prefix().'hrp_customize_staff_payslip_columns');
	...
	{
		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix().'hrp_payslip_pdf_templates');
	...
	/**
	 * get pdf payslip template selected html
	 * @param  [type] $payslip_template_id     
	 * @param  [type] $pdf_payslip_template_id 
	 * @return [type]                          
	 */
	public function get_pdf_payslip_template_selected_html($payslip_template_id, $pdf_payslip_template_id)
	{
		...
		if(isset($pdf_payslip_template_id) && $pdf_payslip_template_id != ''){
			...
			$template_options .= '<option value="0">-</option>';
	...
	/**
	 * hr payroll get payslip pdf only for pdf
	 * @param  string  $id         
	 * @param  array   $where      
	...
	public function hr_payroll_get_payslip_pdf_only_for_pdf($id = '', $where = [], $for_editor = false)
	{
		...
		$company_name = get_default_company_name(get_default_company_id());

		if (is_numeric($id)) {
			$builder = $this->db->table(get_db_prefix().'hrp_payslip_details');
			$builder->where(db_prefix() . 'hrp_payslip_details.id', $id);
			$hrp_payslip_details = $builder->get()->getRow();
			...
			if ($hrp_payslip_details) {
				...
				// get payslip
				$hrp_payslip = $this->get_hrp_payslip($hrp_payslip_details->payslip_id);
				if ($hrp_payslip && is_numeric($hrp_payslip->pdf_template_id)) {
					...
					if ($pdf_payslip_template) {
						...
						if ($for_editor == false) {
							...
							$merge_fields = $payslip_available_variables['payslip_column_keys'];

							$logo_url = '';
							foreach ($merge_fields as $key) {
								if($key == '{LOGO_URL}'){
									$val = '<img src="'.get_file_from_setting("invoice_logo", true).'" />';
								}elseif($key == '{PAYSLIP_RANGE}'){
									$val = $payslip_range;
								}elseif($key == '{COMPANY_NAME}'){
									$val = $company_name;
								}else{
									$val = staff_payslip_map_variables($key, $hrp_payslip_details);
								}
	...
	public function daily_time_synchronization($data)
	{
		$affectedRows = 0;
		$rel_type = hrp_get_timesheets_status();

		$attendance_month = date('Y-m-d',strtotime($data['attendance_fill_month'].'-01'));
		$timesheets_data = $this->get_time_clock_data($attendance_month, $rel_type);
		//get day header in month
		$days_header_in_month = $this->get_day_header_in_month($attendance_month, $rel_type);
		$header_key = array_merge($days_header_in_month['staff_key'], $days_header_in_month['days_key'], $days_header_in_month['attendance_key']);
		...
		if(count($employees_timesheet_leaves) > 0){
			...
			if(count($insert_batch_employees_timesheet) > 0){
				$builder = $this->db->table(get_db_prefix().'hrp_employees_timeshee_leaves');
				$builder->updateBatch( $insert_batch_employees_timesheet, 'id');
			}
		}else{
			foreach ($staff_timesheet_leave_data as $key => $batch_employees_timesheet) {
				$staff_timesheet_leave_data[$key]['staff_id'] = $key;
			...
			$builder = $this->db->table(get_db_prefix().'hrp_employees_timeshee_leaves');
			$builder->insertBatch($staff_timesheet_leave_data);
		}
	 ...
	 * @return [type]           
	 */
	public function get_time_clock_data($month, $rel_type)
	{
		$staff_timesheet_leave_data=[];
		$staff_timesheets=[];
		$staff_timesheet_details=[];
		$standard_working_time = get_setting('standard_working_time');
		...
		$sql_where = 'SELECT *, user_id as staff_id FROM '.get_db_prefix().'attendance
		where date_format(out_time, "%Y-%m") = "'.$y_month.'" AND date_format(in_time,"%Y-%m") = "'.$y_month.'" AND deleted = 0 ;';
		$timesheets = $this->db->query($sql_where)->getResultArray();

		// handle leave data
		$sql_where = 'SELECT *, applicant_id as staff_id, total_hours as value FROM '.get_db_prefix().'leave_applications
		where date_format(start_date, "%Y-%m") = "'.$y_month.'" AND date_format(end_date,"%Y-%m") = "'.$y_month.'" AND deleted = 0 AND status = "approved" ;';
		$leave_applications = $this->db->query($sql_where)->getResultArray();

		foreach ($leave_applications as $leave_application) {
			$timesheet_rel_type ='';
			$date_work = date('d', strtotime($leave_application['start_date']));

			$timesheet_rel_type .= 'paid_leave';
			$column_name = $date_to_column_name[$date_work];

			if(isset($staff_timesheet_leave_data[$leave_application['staff_id']][$column_name])){
				$temp = $staff_timesheet_leave_data[$leave_application['staff_id']][$column_name];
				$staff_timesheet_leave_data[$leave_application['staff_id']][$column_name] = $temp.';PL:'.(float)$leave_application['value'];
			}else{
				$staff_timesheet_leave_data[$leave_application['staff_id']][$column_name] =  'PL:'.(float)$leave_application['value'];
			}

			if(isset($staff_timesheet_leave_data[$leave_application['staff_id']]['paid_leave'])){
				$staff_timesheet_leave_data[$leave_application['staff_id']]['paid_leave'] += (float)$leave_application['value'];
			}else{
				$staff_timesheet_leave_data[$leave_application['staff_id']]['paid_leave'] = (float)$leave_application['value'];
			}

			if($timesheet_rel_type != ''){
				if(isset($staff_timesheets[$leave_application['staff_id']])){
					$staff_timesheets[$leave_application['staff_id']][$timesheet_rel_type] += (float)$leave_application['value'];
				}else{
					$staff_timesheets[$leave_application['staff_id']] =  [
						...
						'unpaid_leave' 		=> 0,
						'rel_type' 			=> $rel_type,
						'standard_workday' 			=> $standard_working_time,
		...
		// handle attendance data
		foreach ($timesheets as $timesheet) {
			$duration = (abs(strtotime($timesheet['out_time']) - strtotime($timesheet['in_time'])));
			$timesheet_value =  round($duration/3600, 2);
			$date_work = date('d', strtotime($timesheet['in_time']));
			...
			if($timesheet_rel_type != ''){
				...
				if(isset($staff_timesheets[$timesheet['staff_id']])){
					$staff_timesheets[$timesheet['staff_id']][$timesheet_rel_type] += (float)$timesheet_value;
				}else{
					$staff_timesheets[$timesheet['staff_id']] =  [
						'staff_id' 			=> $timesheet['staff_id'],
						'month' 			=> $month,
			...
			if($timesheet_rel_type == 'actual_workday'){
				...
				if(isset($staff_timesheet_details[$timesheet['staff_id']][$column_name])){
					$staff_timesheet_details[$timesheet['staff_id']][$column_name] += (float)$timesheet_value;
				}else{
					$staff_timesheet_details[$timesheet['staff_id']][$column_name] =  (float)$timesheet_value;
