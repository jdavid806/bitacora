...
foreach ($campaigns as $campaign) {
  ...
  $item = $exising_items . ' <a href="'.get_uri('ma/campaign_detail/'.$campaign['id']).'" class="kanban-item d-block " data-id="'.$campaign['id'].'">
  <span style="color: '. $campaign['color'] .'">'.$campaign['name'].'</span>
  <div class="clearfix">
    <div class="mt10 font-12 float-start">'.$published.'</div>
      <div class="mt10 font-12 float-end" title="'._l('total_lead').'"><i data-feather="users" class="icon-16"></i> '.count($Ma_model->get_lead_by_campaign($campaign['id'])).'</div>
  </div>
 </a>';
...
<?php require 'plugins/Ma/assets/js/campaigns/kanban_view_js.php';?>
