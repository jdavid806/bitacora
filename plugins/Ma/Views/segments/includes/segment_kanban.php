...
foreach ($segments as $segment) {
  ...
  $item = $exising_items . ' <a href="'.get_uri('ma/segment_detail/'.$segment['id']).'" class="kanban-item d-block " data-id="'.$segment['id'].'">
  <span style="color: '. $segment['color'] .'">'.$segment['name'].'</span>
  <div class="clearfix">
    <div class="mt10 font-12 float-start">'.$published.'</div>
      <div class="mt10 font-12 float-end" title="'._l('total_lead').'"><i data-feather="users" class="icon-16"></i> '.count($Ma_model->get_lead_by_segment($segment['id'])).'</div>
  </div>
 </a>';
...
<?php require 'plugins/Ma/assets/js/segments/kanban_view_js.php';?>
