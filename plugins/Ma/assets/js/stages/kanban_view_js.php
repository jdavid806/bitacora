...
<script type="text/javascript">
    ...
    saveAssignedAndSort = function ($item, category) {
      "use strict";
        ...
        $.ajax({
            url: '<?php echo_uri("ma/stage_change_category") ?>',
            type: "POST",
    ...
    function update_counts() {
        ...
        <?php foreach ($data as $category) { ?>
            $('.<?php echo html_entity_decode($category['id']); ?>-stage-count').html($('.kanban-<?php echo html_entity_decode($category['id']); ?>').find('.kanban-item').length);
        <?php } ?>
    }
