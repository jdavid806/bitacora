...
trait Whatsapp
{
    ...
    public function sendTemplate($rel_data, $template_data, $type = 'campaign', $fromNumber = null)
    {
        ...
        // Procesar el campo header si es de tipo TEXT
        if ($template_data['header_data_format'] == 'TEXT') {
            $header_data = wbParseText($rel_data, 'header', $template_data, 'array');
        }

        // Procesar el campo body (cuerpo del mensaje)
        $body_data = wbParseText($rel_data, 'body', $template_data, 'array');

        // Reemplazar los campos personalizados por los valores reales
        foreach ($body_data as &$body) {
            // Reemplazo de [[nombre_cliente]]
            if (isset($rel_data['company_name'])) {
                $body = str_replace('[[NOMBRE_CLIENTE]]', $rel_data['company_name'], $body);
            }

            // Reemplazo de [[ESPECIALIDAD]]
            if (isset($rel_data['especialidad'])) {
                $body = str_replace('[[ESPECIALIDAD]]', $rel_data['especialidad'], $body);
            }

            // Reemplazo de [[PAIS]]
            if (isset($rel_data['country'])) {
                $body = str_replace('[[PAIS]]', $rel_data['country'], $body);
            }

            // Reemplazo de [[PRODUCTO_DE_INTERES]]
            if (isset($rel_data['productos_interes'])) {
                $body = str_replace('[[PRODUCTO_DE_INTERES]]', $rel_data['productos_interes'], $body);
            }

            // Reemplazo de [[VENDEDOR]]
            if (isset($rel_data['owner_id'])) {
                $body = str_replace('[[VENDEDOR]]', $rel_data['owner_id'], $body);
            }
        }

        // Procesar el campo footer (botones)
        ...
        // Cargar la API de WhatsApp y enviar la plantilla
