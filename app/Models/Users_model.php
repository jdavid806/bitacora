...
class Users_model extends Crud_model
{
    ...
    function get_access_info($user_id = 0)
    {
        ...
        $sql = "SELECT $users_table.id, $users_table.user_type, $users_table.is_admin, $users_table.role_id, $users_table.email, $users_table.skype,
    ...
    {
        $users_table = $this->db->prefixTable('users');
        $user_id = $this->_get_clean_value(options_or_value: $user_id);

        $sql = "SELECT $users_table.*
        FROM $users_table
        WHERE $users_table.deleted=0 AND FIND_IN_SET(':$user_id:') limit 1";
        return $this->db->query($sql);
    }

    public function tiene_acceso_a_cliente($user_id, $client_id)
    {
        // Lógica para verificar si el usuario tiene acceso al cliente
        // Por ejemplo, verificar si el usuario está asociado al cliente
