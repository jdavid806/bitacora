...
class Events_model extends Crud_model
{
    ...
    function get_details($options = array())
    {
        ...
        $sql = "SELECT $events_table.*, $select
            CONCAT($users_table.first_name, ' ',$users_table.last_name) AS created_by_name, $users_table.image AS created_by_avatar, $users_table.skype, $clients_table.company_name, $clients_table.is_lead, $select_labels_data_query
        FROM $events_table
    ...
    function toggle_event_attended_status($id)
    {
        $events_table = $this->db->prefixTable('events');

        $id = $this->_get_clean_value($id);

        // Get the current attended_demo status
        $sql = "SELECT attended_demo FROM $events_table WHERE $events_table.id=$id";
        $result = $this->db->query($sql)->getRow();

        if ($result) {
            $current_status = $result->attended_demo;
            $new_status = $current_status == 1 ? 0 : 1;

            // Update the attended_demo status
            $sql = "UPDATE $events_table SET attended_demo = $new_status WHERE $events_table.id=$id";
            return $this->db->query($sql);
        }
    ...
    function get_events_for_user($user, $start_date, $start_time, $end_time)
    {
        $events_table = $this->db->prefixTable('events');

        $sql = "SELECT * FROM $events_table 
    WHERE (created_by = ? OR share_with LIKE CONCAT('%', ?, '%')) 
    AND start_date = ? 
    AND (
        (
            (start_time < ? AND (end_time > ? OR (end_time IS NULL AND DATE_ADD(start_time, INTERVAL 1 HOUR) > ?)))
        ) OR   -- El evento existente cubre el nuevo rango, o cubre al menos una hora si end_time es NULL
        (
            (start_time < ? AND (end_time > ? OR (end_time IS NULL AND DATE_ADD(start_time, INTERVAL 1 HOUR) > ?)))
        ) OR   -- El inicio del nuevo rango cae dentro de un evento existente, o cubre al menos una hora si end_time es NULL
        (
            (start_time >= ? AND start_time < ?)
        )      -- Un evento existente cae dentro del nuevo rango
    )";
