...
class Projects_model extends Crud_model
{
    ...
    function get_projects_id_and_name($options = array())
    {

        $projects_table = $this->db->prefixTable('projects');
        $where = "";

        $project_type = $this->_get_clean_value($options, "project_type");

        if ($project_type) {

            $where .= " AND $projects_table.project_type='$project_type'";
        }

        $sql = "SELECT id, title, project_type

        FROM $projects_table 

        WHERE $projects_table.deleted=0 AND $projects_table.status_id=1 $where";
