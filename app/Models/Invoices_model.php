...
class Invoices_model extends Crud_model
{
    ...
    function get_details($options = array())
    {
        ...
        $due_date_interval = $this->_get_clean_value($options, "due_date_interval");
        if ($due_date_interval > 0) {
            $where .= " AND $invoices_table.due_date BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL $due_date_interval DAY)";
        }
        ...
        $recurring_invoices = $this->_get_clean_value($options, "recurring_invoices");
        if ($recurring_invoices) {
            $where .= " AND $invoices_table.recurring_invoice_id>=0";
        }
        ...
        $sql = "SELECT $invoices_table.*, $clients_table.currency, $clients_table.currency_symbol, $clients_table.company_name, $clients_table.cod_cliente, $projects_table.title AS project_title, credit_note_table.id AS credit_note_id, credit_note_table.display_id AS credit_note_display_id, main_invoice_table.display_id AS main_invoice_display_id, recurring_invoice_table.display_id AS recurring_invoice_display_id,
