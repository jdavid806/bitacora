...
class Tickets_model extends Crud_model
{
    ...
    function get_details($options = array())
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $tickets_table.*,$ticket_types_table.title AS ticket_type, 
                $clients_table.company_name,$project_table.title AS project_title, 
                $task_table.title AS task_title, 
                CONCAT(assigned_table.first_name, ' ', assigned_table.last_name) AS assigned_to_user, 
                assigned_table.image AS assigned_to_avatar, $select_labels_data_query $select_custom_fieds,
                CONCAT(COALESCE(requested_table.first_name, ''), ' ', COALESCE(requested_table.last_name, '')
                ) AS requested_by_name,
                (SELECT COUNT(*) FROM $task_table WHERE $task_table.ticket_id = $tickets_table.id 
                AND $task_table.status IN ('to_do', 'in_progress')) AS task_count,
                (SELECT SEC_TO_TIME(SUM(TIMESTAMPDIFF(SECOND, $task_table.start_date, $task_table.deadline)))
                FROM $task_table WHERE $task_table.ticket_id = $tickets_table.id 
                AND $task_table.status IN ('to_do', 'in_progress')) AS total_estimated_time
                FROM $tickets_table
    ...
    function get_details_refactor($options = array())
    {
        ...
        $status = $this->_get_clean_value($options, "status");
        if ($status === "closed") {
            $where .= " AND $tickets_table.status = 'closed'";
        }
        if ($status === "open") {
            $where .= " AND $tickets_table.status IN ('new', 'open', 'client_replied')";
        }
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $tickets_table.title, $tickets_table.id , $ticket_types_table.title AS ticket_type, $tickets_table.created_at AS created_at, $tickets_table.closed_at AS closed_at, $tickets_table.status AS status,
                $clients_table.company_name AS client, CONCAT(created_by.first_name, ' ', created_by.last_name) AS user_created,
                $task_table.title AS task_title, 
                CONCAT(assigned_table.first_name, ' ', assigned_table.last_name) AS assigned_to_user, 
                ...
                LEFT JOIN $users_table AS assigned_table ON assigned_table.id = $tickets_table.assigned_to
                LEFT JOIN $users_table AS requested_table ON requested_table.id = $tickets_table.requested_by
                LEFT JOIN $users_table AS created_by ON created_by.id = $tickets_table.created_by
                LEFT JOIN $project_table ON $project_table.id = $tickets_table.project_id
                LEFT JOIN $task_table ON $task_table.id = $tickets_table.task_id
                $join_custom_fieds
                WHERE $tickets_table.deleted = 0 $where $custom_fields_where
    ...
    function get_details_reduced($options = array())
    {
        $tickets_table = $this->db->prefixTable('tickets');
        $ticket_types_table = $this->db->prefixTable('ticket_types');
        $clients_table = $this->db->prefixTable('clients');
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS $tickets_table.title, $tickets_table.id, $tickets_table.created_at AS created_at, $tickets_table.closed_at AS closed_at, $tickets_table.status AS status, $ticket_types_table.title AS ticket_type,
                CONCAT(assigned_table.first_name, ' ', assigned_table.last_name) AS assigned_to_user, 
                $select_labels_data_query $select_custom_fieds,
                CONCAT(COALESCE(requested_table.first_name, ''), ' ', COALESCE(requested_table.last_name, '')
                ) AS requested_by_name,
    ...
    function get_tickets_by_range_dates($start_date, $end_date)
    {
        $tickets_table = $this->db->prefixTable('tickets');
        $sql = "SELECT $tickets_table.*
        FROM $tickets_table
        WHERE $tickets_table.deleted=0 AND DATE($tickets_table.created_at) >= '$start_date' AND DATE($tickets_table.created_at) <= '$end_date'";
