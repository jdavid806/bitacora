...
class Clients_model extends Crud_model
{
    ...
    function get_all_clients($options = array())
    {
    ...
    function set_descarted($client_id, $campo, $valor)
    {

        $clients_table = $this->db->prefixTable('clients');

        $campo = $this->db->escapeString($campo);
        $valor = $this->db->escape($valor); // Usamos escape para valores

        // Construye la consulta SQL
        $sql = "UPDATE $clients_table SET {$campo} = {$valor} WHERE id = {$client_id}";
    ...
    function get_is_discard($user_id)
    {
        // Obtener el nombre de la tabla de clientes
        $clients_table = $this->db->prefixTable('clients');

        // Limpiar el valor de user_id para evitar SQL injection
        $user_id = $this->_get_clean_value($user_id);

        // Preparar la consulta SQL para verificar si el cliente ha sido descartado
        $sql = "SELECT descartado FROM $clients_table
            WHERE deleted = 0 AND id = ?
            LIMIT 1";

        // Ejecutar la consulta con un parámetro preparado
        $query = $this->db->query($sql, [$user_id]);
        $result = $query->getRow();

        // Verificar si se encontró un resultado y si el cliente ha sido descartado
        if ($result && $result->descartado == 1) {
            return true; // Cliente descartado
        }

        return false; // Cliente no descartado o no encontrado
    }
    ...
    function get_by_ids($clients_ids_csv)
    {
        $messages_table = $this->db->prefixTable('clients');

        $sql = "SELECT $messages_table.* FROM $messages_table WHERE id in ($clients_ids_csv)";

        return $this->db->query($sql);
    }

    function get_clients_by_scheduling_and_demo(
        $country,
        $isLead,
        $created_since,
        $start_date,
        ...
        $countryCondition = $country ? "AND c.country = '$country'" : "";
        $isLeadCondition = $isLead !== null ? "AND c.is_lead = $isLead" : "";
        $createdSinceCondition = $created_since ? "AND c.created_date >= '$created_since'" : "";
        $startDateCondition = $start_date ? "AND e.start_date >= '$start_date'" : "";
        $endDateCondition = $end_date ? "AND e.start_date <= '$end_date'" : "";

        if ($hasScheduling == 1) {
            $attendedCondition = $attended == 1
                ? "AND e.attended_demo = 1"
                : "AND e.attended_demo = 0";
            ...
            $sqlEntry = "SELECT 
                ...
                c.phone AS telefono,
                c.email AS correo,
                c.state AS departamento,
                c.city AS ciudad,
                c.country AS pais,
                c.is_lead AS es_lead,
                c.created_date AS created_at,
                MIN(e.start_date) AS event_start_date,
                MIN(e.start_time) AS event_start_time,
                e.attended_demo AS event_attended
            FROM crm_clients AS c
            JOIN crm_events AS e ON e.client_id = c.id
            WHERE e.deleted = 0 
            $countryCondition 
            $isLeadCondition 
            $createdSinceCondition 
            $startDateCondition 
            $endDateCondition
            GROUP BY c.id";
            ...
            $sqlEntry = "SELECT 
                ...
                c.created_date AS created_at
            FROM crm_clients AS c
            LEFT JOIN crm_events AS e ON e.client_id = c.id
            WHERE e.id IS NULL 
            $countryCondition 
            $isLeadCondition 
            $createdSinceCondition";
        ...
        $entryQuery = $this->db->query($sqlEntry);
        $entryClients = $entryQuery->getResult();

        $exitClients = [];
        if (!empty($sqlExit)) {
            $exitQuery = $this->db->query($sqlExit);
            $exitClients = $exitQuery->getResult();
        }

        return [
            'entry_clients' => $entryClients,
            'exit_clients' => $exitClients
        ];
    }

    function get_clients_by_demo()
    {
        $sql = "SELECT 
            c.id as client_id,
            c.company_name as nombre_cliente,
            ...
            e.attended_demo as event_attended
        FROM crm_clients as c
        LEFT JOIN crm_events e ON e.client_id = c.id
        WHERE e.deleted = 0";
    ...
    function get_clients_by_demo_and_estimate(
        $country,
        $isLead,
        $created_since,
        $demo_start_date,
        ...
        $isLeadCondition = $isLead !== null ? "AND c.is_lead = $isLead" : "";
        $createdSinceCondition = $created_since ? "AND c.created_date >= '$created_since'" : "";
        $demoStartDateCondition = $demo_start_date ? "AND e.start_date >= '$demo_start_date'" : "";
        $demoEndDateCondition = $demo_end_date ? "AND e.start_date <= '$demo_end_date'" : "";

        if ($attendedDemo == 1) {
            $demoCondition = "AND e.attended_demo = 1";
        } else {
            $demoCondition = "AND (e.attended_demo = 0 OR e.id IS NULL)";
        }

        if ($hasEstimate == 1) {
            $estimateCondition = "AND est.id IS NOT NULL";
        } else {
            $estimateCondition = "AND est.id IS NULL";
        }

        // Consulta de clientes de entrada
        $sqlEntry = "SELECT 
            c.id AS binnacle_id,
            c.company_name AS nombre_cliente,
            c.phone AS telefono,
        ...
        $isLeadCondition 
        $createdSinceCondition 
        $demoStartDateCondition 
        $demoEndDateCondition
        $demoCondition
        GROUP BY c.id";

        // Consulta de clientes de salida
        $sqlExit = "SELECT 
            c.id AS binnacle_id,
            c.company_name AS nombre_cliente,
            c.phone AS telefono,
            ...
            c.is_lead AS es_lead,
            c.created_date AS created_at,
            MAX(est.id) AS last_estimate_id,
            MAX(est.estimate_date) AS last_estimate_date,
            est.status AS estimate_status
        FROM crm_clients AS c
        LEFT JOIN crm_estimates AS est ON est.client_id = c.id
        WHERE c.id IN (
            SELECT DISTINCT c.id
            FROM crm_clients AS c
            LEFT JOIN crm_events AS e ON e.client_id = c.id
    ...
    function get_clients_by_estimate_and_purchase(
        $country,
        $isLead,
        $created_since,
        $estimate_start_date,
        ...
        $isLeadCondition = $isLead !== null ? "AND c.is_lead = $isLead" : "";
        $createdSinceCondition = $created_since ? "AND c.created_date >= '$created_since'" : "";
        $estimateStartDateCondition = $estimate_start_date ? "AND est.estimate_date >= '$estimate_start_date'" : "";
        $estimateEndDateCondition = $estimate_end_date ? "AND est.estimate_date <= '$estimate_end_date'" : "";
        $estimateDateConditions = "$estimateStartDateCondition $estimateEndDateCondition";
        ...
        if ($hasPurchase == 1) {
            $purchaseCondition = "AND ip.id IS NOT NULL";
        } else {
            $purchaseCondition = "AND ip.id IS NULL";
        }
        ...
        $sqlExit = "SELECT 
            ...
            est.estimate_date AS estimate_date,
            est.valid_until AS estimate_valid_until,
            est.status AS estimate_status,
            inv.id AS invoice_id,
            inv.invoice_total AS invoice_total,
            inv.invoice_subtotal AS invoice_subtotal,
            inv.status AS invoice_status,
            ip.id AS payment_id,
            ip.amount AS payment_amount,
            ip.payment_date AS payment_date,
            ip.payment_method_id AS payment_method
        FROM crm_clients AS c
        LEFT JOIN crm_estimates AS est ON est.client_id = c.id
        LEFT JOIN crm_invoices AS inv ON inv.client_id = c.id AND inv.deleted = 0
        LEFT JOIN crm_invoice_payments AS ip ON ip.invoice_id = inv.id AND ip.deleted = 0
        WHERE c.id IN (
            SELECT DISTINCT c.id
            FROM crm_clients AS c
            LEFT JOIN crm_estimates AS est ON est.client_id = c.id
