...
class Attendance_model extends Crud_model
{
    ...
    function log_time_anterior($user_id, $note = "")
    {
    ...
    function log_time($user_id, $note = "", $type = "", $photo = "")
    {
    ...
    function get_clock_in_out_details_of_all_users($options = array())
    {
        ...
        $sql = "SELECT CONCAT($users_table.first_name, ' ',$users_table.last_name) AS member_name, $users_table.image, $users_table.id, attendance_table.id AS attendance_id, attendance_table.in_time, attendance_table.out_break_time,attendance_table.in_break_time,attendance_table.photo
        FROM $users_table
        LEFT JOIN (SELECT user_id, id, in_time, out_break_time,in_break_time, photo FROM $attendnace_table WHERE $attendnace_table.deleted=0 AND $attendnace_table.status='incomplete') AS attendance_table ON attendance_table.user_id=$users_table.id
