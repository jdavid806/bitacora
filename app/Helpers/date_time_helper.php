...
if (!function_exists('convert_date_utc_to_local')) {
    ...
    {
        $date_time = str_replace("-undefined-undefined", "", $date_time);
...
if (!function_exists('format_time_to_days_hours')) {
    function format_time_to_days_hours($time)
    {
        // Verifica si el tiempo es nulo o vacío
        if (is_null($time) || $time === '') {
            return app_lang("without_estimate");
        }

        // Divide el tiempo en partes (horas, minutos, segundos)
        ...
        $days = floor($total_minutes / 1440); // 1440 minutos en un día
        ...
        $formatted_time = [];
        if ($days > 0) {
            $formatted_time[] = "$days días";
        }
        if ($remaining_hours > 0) {
            $formatted_time[] = "$remaining_hours horas";
        }
        if ($remaining_minutes > 0) {
            $formatted_time[] = "$remaining_minutes minutos";
        }

        // Une las partes en un solo string
        return implode(' y ', $formatted_time);
    }
}
