...
if (!function_exists('logged_user_can_approve_budget')) {

    function logged_user_can_approve_budget()
    {
        $config = get_setting('can_approve_budget_users');
        $user_ids = explode(",", $config);
        $ci = new Security_Controller(false);
        return in_array($ci->login_user->id, $user_ids);
    }
}
...
?>
                    <img class="max-logo-size"
                        src="<?php echo get_source_url_of_file($file, get_setting("system_file_path"), "thumbnail", $only_file_path, $only_file_path); ?>"
                        alt="..." />
                <?php
                }
            }
...
if (!function_exists('convertHtmlToPlainText')) {
    function convertHtmlToPlainText($html)
    {
        ...
        // Eliminar cualquier otra etiqueta HTML que pueda quedar
...
if (!function_exists('replace_client_data_in_template')) {
    function replace_client_data_in_template($client, $template_content)
    {
        $session = new Security_Controller();
        $logged_in_user = $session->login_user;
        $template_content = str_replace('[[NOMBRE_CLIENTE]]', $client->company_name, $template_content);
        $template_content = str_replace('[[ESPECIALIDAD]]', $client->especialidad, $template_content);
        $template_content = str_replace('[[PAIS]]', $client->country, $template_content);
        $template_content = str_replace('[[VENDEDOR]]', $logged_in_user->first_name . " " . $logged_in_user->last_name, $template_content);

        $items = $session->Items_model->get_details(array("ids" => $client->productos_interes))->getResult();
        $items_names = array();
        foreach ($items as $key => $item) {
            array_push($items_names, $item->title);
        }
        $template_content = str_replace('[[PRODUCTO_DE_INTERES]]', implode(", ", $items_names), $template_content);
...
if (!function_exists('save_message')) {
    function save_message($client, $user_id, $content, $files_data)
    {
        $ci = new App_Controller();
        if ($client->id >= 1) {
            $content = replace_client_data_in_template($client, $content);
        }
        ...
        return $ci->Client_messages_model->ci_save($data);
    }
}
