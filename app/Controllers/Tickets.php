...
class Tickets extends Security_Controller
{
    ...
    function modal_form()
    {
        ...
        $view_data['clients_dropdown'] = array("" => "-") + $this->Clients_model->get_dropdown_list(array("company_name", "cod_cliente"), "id", array("is_lead" => 0));
    ...
    function save()
    {
        ...
        $arr_elegible_users = $this->elegible_users($ticket_type_id);
        ...
        $ticket_data = array(
            ...
            "assigned_to" => $assigned_to ? $assigned_to : (count($arr_elegible_users) ? $arr_elegible_users[array_rand($arr_elegible_users)]->user_id : 0),
        ...
        if ($ticket_id) {
            ...
            // $this->ticket_created_notify($client_id, $ticket_id, $assigned_to, "created");
    ...
    function elegible_users($ticket_type_id)
    {
        $count_tickets_by_users = $this->Tickets_settings_model->count_tickets_by_setting()->getResult();

        $eligible_users = [];
        foreach ($count_tickets_by_users as $user) {

            $allowed_ticket_types = explode(', ', $user->ticket_type_ids);
            if (!in_array($ticket_type_id, $allowed_ticket_types)) {
                continue;
            }

            if ($user->total_tickets > $user->total_max_tickets) {
                continue;
            }

            if ($user->total_tickets > round($user->avg_total_tickets)) {
                continue;
            }
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        $row_data = array(
            ...
            format_time_to_days_hours($data->total_estimated_time),
        ...
        if ($this->login_user->user_type == "staff") {
            ...
            if ($data->status === "closed") {
                ...
                $status = '<li role="presentation">' . js_anchor("<i data-feather='check-circle' class='icon-16'></i> " . app_lang('mark_as_closed'), array('title' => app_lang('mark_as_closed'), "class" => "dropdown-item", "data-action-url" => get_uri("tickets/save_ticket_status/$data->id/closed/$data->client_id/$data->assigned_to"), "data-action" => "update")) . '</li>';
            }
    ...
    function save_ticket_status($ticket_id = 0, $status = "closed", $client_id = 0, $assigned_to = 0)
    {
        ...
        if ($ticket_id) {
            ...
            if ($save_id) {
                ...
                // $this->ticket_created_notify($client_id, $ticket_id, $assigned_to, "closed");
    ...
    function ticket_created_notify($client_id, $ticket_id, $assigned_to, $status)
    {

        $client_info = $this->Clients_model->get_details(array("id" => $client_id))->getRow();
        $ticket_info = $this->Tickets_model->get_details(array("id" => $ticket_id))->getRow();
        $user_info = $this->Users_model->get_details(array("id" => $assigned_to))->getRow();
        $ticket_comment_info = $this->Ticket_comments_model->get_details(array("ticket_id" => $ticket_id))->getRow();
        $is_lead = $client_info->is_lead ? 'lead' : 'client';


        if ($status == "created") {
            $template_name = $client_info->is_lead ? "ticket_created_lead" : "ticket_created_client";
            $whatsapp_template = $this->Templates_model->get_final_template($template_name, true);
        } else {
            $template_name = $client_info->is_lead ? "ticket_closed_lead" : "ticket_closed_client";
            $whatsapp_template = $this->Templates_model->get_final_template($template_name, true);
        }
        ...
        $content = str_replace('{TICKET_ID}', $ticket_info->id, $whatsapp_template['message_default']);
        $content = str_replace('{USER_NAME}', $user_info->first_name . ' ' . $user_info->last_name, $content);
        $content = str_replace('{TICKET_TITLE}', $ticket_info->title, $content);
        $content = str_replace('{TICKET_CONTENT}', $ticket_comment_info->description, $content);

        $content = convertHtmlToPlainText($content);

        $data = array(
            'number' => $client_info->phone,
            'text' => $content,
        );

        $wpp_linkkey_type = $is_lead == "lead" ? "SALES" : "SUPPORT";

        $this->api_evolution_whatsapp($data, $wpp_linkkey_type, "message/sendText", "POST", "sendText");
    }
}
