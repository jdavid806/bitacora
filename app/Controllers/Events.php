...
class Events extends Security_Controller
{
    ...
    function save()
    {
        ...
        $is_reminder = $this->request->getPost('is_reminder') ? $this->request->getPost('is_reminder') : 0;
        $id_conditional = $this->request->getPost('client_id') ? $this->request->getPost('client_id') : $this->request->getPost('lead_id');
        ...
        $client_info = $this->Clients_model->get_details(["id" => $client_id])->getRow();

        $code_country = search_code_by_country($client_info->country);

        $api_key = 'VRCFRYL4FNSF';

        $url = "http://api.timezonedb.com/v2.1/list-time-zone?key=$api_key&format=json&country=$code_country";

        // Hacer la solicitud HTTP
        $response = file_get_contents($url);
        $data_time_zone = json_decode($response, true);

        $arr_time_zone = explode("/", $data_time_zone["zones"][0]["zoneName"]);
        $str_time_zone = [$arr_time_zone[0], $arr_time_zone[1]];
        if (count($arr_time_zone) > 2) {
            $str_time_zone = [$arr_time_zone[0], $arr_time_zone[2]];
        }

        $local_date = new DateTime($start_date . " " . $start_time, new DateTimeZone("America/Bogota"));
        $local_date->setTimezone(new DateTimeZone(implode("/", $str_time_zone)));

        $client_timezone = $local_date;

        $target_path = get_setting("timeline_file_path");
        $files_data = move_files_from_temp_dir_to_permanent_dir($target_path, "event");
        $new_files = unserialize($files_data);

        $created_by = $this->Users_model->get_one($this->login_user->id);
        ...
        $data = array(
            ...
            "end_time" => $end_time,
            "start_time_client" => $client_timezone->format("H:i"),
            "location" => $created_by->skype,
            "labels" => $labels,
        ...
        if ($id) {
            $this->send_event_add_meet_wpp($client_id, $data, $id, $is_reminder);
        } else if (!$is_reminder) {
            $this->send_event_add_meet_wpp($client_id, $data, 0, $is_reminder);
        } else {
            $this->send_event_add_meet_wpp($id_conditional, $data, 0, $is_reminder);
        }
    ...
    function send_event_add_meet_wpp($client_id, $data, $id = null, $is_reminder)
    {
        if ($client_id) {
            ...
            if ($id) {
                $template_internal = $client_info->is_lead ? "modify_meet_lead_internal" : "modify_meet_client_internal";
                $template_external = $client_info->is_lead ? "modify_meet_lead_external" : "modify_meet_client_external";
                $whatsapp_template_internal = $this->Templates_model->get_final_template($template_internal, true);
                $whatsapp_template_external = $this->Templates_model->get_final_template($template_external, true);
            } else {
                $template_internal = $client_info->is_lead ? "add_meet_lead_internal" : "add_meet_client_internal";
                $template_external = $client_info->is_lead ? "add_meet_lead_external" : "add_meet_client_external";
                $whatsapp_template_internal = $this->Templates_model->get_final_template($template_internal, true);
                $whatsapp_template_external = $this->Templates_model->get_final_template($template_external, true);
            }

            $seconds_hour_start_local = strtotime($data['start_time']) - strtotime('TODAY');
            $seconds_hour_end_local = strtotime($data['end_time']) - strtotime('TODAY');
            $seconds_unlocal = strtotime($data['start_time_client']) - strtotime('TODAY');

            $total_seconds_local = $seconds_hour_end_local - $seconds_hour_start_local;

            $seconds_unlocal = $total_seconds_local + $seconds_unlocal;
            $total_hour_unlocal = gmdate('H:i:s', $seconds_unlocal);

            $content_external = str_replace('{CUSTOMER_NAME}', $client_info->company_name, $whatsapp_template_external['message_default']);
            $content_external = str_replace('{LOCATION}', $this->login_user->skype, $content_external);
            $content_external = str_replace('{DATE}', $data["start_date"], $content_external);
            $content_external = str_replace('{HOUR_START}', $data["start_time_client"], $content_external);
            $content_external = str_replace('{HOUR_END}', $total_hour_unlocal, $content_external);
            $content_external = str_replace('{SELLER}', $this->login_user->first_name . " " . $this->login_user->last_name, $content_external);
            $content_external = str_replace('{SPECIALTY}', $client_info->especialidad, $content_external);
            $content_external = str_replace('{REASON_EVENT}', $data["description"], $content_external);
            $content_external = str_replace('{COUNTRY}', $client_info->country, $content_external);


            $content_internal = str_replace('{CUSTOMER_NAME}', $client_info->company_name, $whatsapp_template_internal['message_default']);
            $content_internal = str_replace('{LOCATION}', $this->login_user->skype, $content_internal);
            $content_internal = str_replace('{DATE}', $data["start_date"], $content_internal);
            $content_internal = str_replace('{HOUR_START}', $data["start_time"], $content_internal);
            $content_internal = str_replace('{HOUR_END}', $data["end_time"], $content_internal);
            $content_internal = str_replace('{SELLER}', $this->login_user->first_name . " " . $this->login_user->last_name, $content_internal);
            $content_internal = str_replace('{SPECIALTY}', $client_info->especialidad, $content_internal);
            $content_internal = str_replace('{REASON_EVENT}', $data["description"], $content_internal);
            $content_internal = str_replace('{COUNTRY}', $client_info->country, $content_internal);

            $content_external = convertHtmlToPlainText($content_external);
            $content_internal = convertHtmlToPlainText($content_internal);

            if (!$is_reminder) {
                $this->whatsapp_sent_EA($client_info->phone, $content_external, 'sendText', "SALES");
                foreach ($user_approve_ids as $user_id) {
                    $user_info = $this->Users_model->get_details(array("id" => $user_id))->getRow();
                    $this->whatsapp_sent_EA($user_info->phone, $content_internal, 'sendText', "SUPPORT");
                }
            }
        }
    }
    ...
    function save_event_status()
    {
        ...
        $this->Events_model->save_event_attended_status($event_id, $user_id, $status);
    ...
    function toggle_event_attended_status()
    {
        $encrypted_event_id = $this->request->getPost('encrypted_event_id');
        $event_id = decode_id($encrypted_event_id, "event_id");
        validate_numeric_value($event_id);

        $this->Events_model->toggle_event_attended_status($event_id);
        $new_data = $this->Events_model->get_one($event_id);

        echo json_encode(array(
            "success" => true,
            'message' => app_lang('record_updated'),
            "attended_status" => $new_data->attended_demo,
        ));
    }
    ...
    function reminders($id = 0)
    {
        $info_client = $this->Clients_model->get_details(["id" => $id])->getRow();
        $this->can_create_reminders();
        $view_data["project_id"] = $this->request->getPost("project_id");
        $view_data["client_id"] = !$info_client->is_lead ? $info_client->id : 0;
        $view_data["lead_id"] = $info_client->is_lead ? $info_client->id : 0;
