...
class Tasks extends Security_Controller
{
    ...
    function modal_form()
    {
        ...
        $view_data["users_assign_to"] = array("" => app_lang("automatic_assignment")) + $this->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("user_type" => "staff"));
        ...
        $view_data['projects'] = $this->Ticket_types_model->get_dropdown_list(array("title"), "id", array());;

        $templates = $this->Templates_model->get_details(["template_type" => "tickets"])->getResult();

        $template_options = ["-" => "Seleccione plantilla"];

        foreach ($templates as $template) {
            $template_options[$template->id] = str_replace("_", " ", $template->template_name);
        }
        $view_data['template_options'] = $template_options;


        json_encode(["projects" => $view_data['projects']]);
    ...
    function save()
    {
        ...
        $info_ticket_types = $this->Ticket_types_model->get_details(["id" => $project_id])->getRow();
        $info_project = $this->Projects_model->get_details(["title" => $info_ticket_types->title])->getRow();
        ...
        $arr_elegible_users = $this->elegible_users($info_ticket_types->id);

        $assigned_to = $this->request->getPost('user_id') ? $this->request->getPost('user_id') : (count($arr_elegible_users) ? $arr_elegible_users[array_rand($arr_elegible_users)]->user_id : 0);
        ...
        $info_status_task = $this->Task_status_model->get_details(["key_name" => $status])->getRow();

        $data = array(
            "title" => $this->request->getPost('title'),
            "description" => $this->request->getPost('description'),
            "project_id" => $info_project ? $info_project->id : 0,
            "milestone_id" => $milestone_id ? $milestone_id : 0,
            "points" => $this->request->getPost('points'),
            "status_id" => $info_status_task->id,
    ...
    {
        $count_tasks_by_users = $this->Tickets_settings_model->count_tickets_by_setting()->getResult();

        $eligible_users = [];
        foreach ($count_tasks_by_users as $user) {

            $allowed_task_types = explode(', ', $user->ticket_type_ids);
            if (!in_array($ticket_type_id, $allowed_task_types)) {
                continue;
            }

            if ($user->total_tasks > $user->max_tasks) {
                continue;
            }

            if ($user->total_tasks > round($user->avg_total_tasks)) {
                continue;
            }
