...
class Subscriptions extends Security_Controller
{
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        $row_data = array(
            ...
            anchor(get_uri("clients/view/" . $data->client_id), $data->cod_cliente),
            $data->bill_date,
    ...
    function get_send_subscription_template_wpp($subscription_id)
    {

        $sub_info = $this->Subscriptions_model->get_details(array("id" => $subscription_id))->getRow();
        $is_lead = $sub_info->is_lead ? 'lead' : 'client';
        $sub_info->contacts = $this->Users_model->get_details(array("user_type" => $is_lead, "client_id" => $sub_info->client_id))->getResult();
        $whatsapp_template = $this->Templates_model->get_final_template("subscription_request_sent_client", true);

        $contact_info = "";
        foreach ($sub_info->contacts as $contact) {
            if ($contact->is_primary_contact) {
                $contact_info = $contact;
            }
        }

        $parser_data["SUBSCRIPTION_ID"] = $sub_info->id;
        $parser_data["SUBSCRIPTION_TITLE"] = $sub_info->title;
        $parser_data["CONTACT_FIRST_NAME"] = $contact_info->first_name;
        $parser_data["CONTACT_LAST_NAME"] = $contact_info->last_name;
        $parser_data["SUBSCRIPTION_URL"] = get_uri("subscriptions/prepare_data_to_preview/" . $subscription_id);
        $parser_data['LOGO_URL'] = '';
        $parser_data["SIGNATURE"] = '';
        $parser_data["RECIPIENTS_EMAIL_ADDRESS"] = $sub_info->email;
        ...
        $wpp_linkkey_type = $sub_info->is_lead == 1 ? "SALES" : "";

        $this->Whatsapp_sent($sub_info->phone, $content, $wpp_linkkey_type);
    }

    function prepare_data_to_preview($subscription_id = 0)
    {
        ...
        foreach ($view_data['payment_methods'] as $payment_method) {

            $method_type = get_array_value($payment_method, "type");

            if ($view_data['subscription_total_summary']->balance_due >= get_array_value($payment_method, "minimum_payment_amount")) {
                if ($method_type == "stripe") {

                    $url = get_uri("subscriptions/get_stripe_checkout_session_test");
                    $currency = $view_data['subscription_total_summary']->currency;
                    $balance_due = $view_data['subscription_total_summary']->balance_due;

                    // Datos que quieres enviar en la petición POST
                    $data = [
                        'input_data' => [
                            'subscription_id' => $subscription_id,
                            'payment_amount' => to_decimal_format($balance_due),
                            'verification_code' => isset($verification_code) ? $verification_code : "",
                            'contact_user_id' => isset($contact_user_id) ? $contact_user_id : "",
                            'currency' => $currency,
                            'balance_due' => $balance_due,
                            'client_id' => $view_data['subscription_info']->client_id,
                            'payment_method_id' => get_array_value($payment_method, "id"),
                            'description' => app_lang("pay_subscription") . ':' . to_currency($balance_due, $currency . " "),
                        ]
                    ];

                    $result = $this->get_stripe_checkout_session_on_controller($data);

                    $responseArray = json_decode($result, true);

                    if ($responseArray['success'] && $responseArray['checkout_url']) {
                        echo '<script type="text/javascript"> window.location.href ="' . $responseArray['checkout_url'] . '"
                              </script>';
                    }
                }
            }
    ...
    function get_stripe_checkout_session_on_controller($data = [])
    {
        $stripe = new Stripe();

        try {
            $session = $stripe->get_stripe_checkout_session($data['input_data'], $this->login_user->id);
            if ($session->id) {
                return json_encode(array("success" => true, "checkout_url" => $session->url));
            } else {
    ...
    function activate_as_stripe_subscription()
    {
        ...
        $notification_setting_info = $this->Notification_settings_model->get_notify_to_users_of_event("subscription_request_sent");

        if ($notification_setting_info->enable_whatsapp) {
            $this->get_send_subscription_template_wpp($subscription_id);
        }
