...
class Projects extends Security_Controller {
    ...
    function all_projects_client($client_id = null) {
        $status_id = 0;
        if (!$client_id) {
            die("Error en código de cliente");
        }

        function urlSafeBase64Decode($data){
            // Revertir los cambios de la codificación y decodificar
            $data = strtr($data, '-_', '+/');
            $mod4 = strlen($data) % 4;
            if ($mod4) {
                $data .= substr('====', $mod4); // Agregar los "=" faltantes
            }
            return base64_decode($data);
        }

        // $client_id = base64_decode(base64_decode($client_id));
        $client_id = urlSafeBase64Decode($client_id);
        ...
        validate_numeric_value($status_id);
        $view_data['project_labels_dropdown'] = json_encode($this->make_labels_dropdown("project", "", true));
        $view_data["can_create_projects"] = false;
        $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("projects", $this->login_user->is_admin, $this->login_user->user_type);
        $view_data["custom_field_filters"] = $this->Custom_fields_model->get_custom_field_filters("projects", $this->login_user->is_admin, $this->login_user->user_type);
        $view_data["selected_status_id"] = $status_id;
        $view_data['project_statuses'] = $this->Project_status_model->get_details()->getResult();
    
        // Asegúrate de pasar el client_id al view_data para ambos casos
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            return $this->template->rander("projects/projects_view_client_b", $view_data);
        } else {
            if (!$this->can_client_access("project", false)) {
                app_redirect("forbidden");
            }
            
            $view_data['client_id'] = $client_id; // Asegurar el client_id aquí también
            $view_data["can_edit_projects"] = false;
            $view_data["can_delete_projects"] = false;
            
            // $view_data['page_type'] = "full";
            return $this->template->rander("clients/projects/project_clients_b", $view_data);
        }
    }
    ...
    //     $status_id = 0;
    //     if (!$client_id) {
    //         die("Error en codigo de cliente");
    //     }
    ...
    function projects_list_data_of_client($client_id = 0) {
        ...
        // echo "Devolviendo";
        // var_dump(json_encode(array("data" => $result)));
