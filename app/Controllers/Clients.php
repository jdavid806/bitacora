...
class Clients extends Security_Controller
{
    ...
    function index2($tab = "")
    {
        ...
        return $this->template->rander("clients/index2", $view_data);
    }
    ...
    function licenses_services()
    {
        $view_data["user_info"] = $this->Users_model->get_one($this->login_user->id);
        $view_data["instance_info"] = $this->Api_evolution_instances_model->get_details(["client_id" => $view_data["user_info"]->client_id])->getRow();
        ...
        return $this->template->view("clients/contacts/licenses_services", $view_data);
    }
    ...
    // TODO -> DEIVYD: BORRAR ESTO, SOLO ES PARA PRUEBAS
    function clients_list_2()
    {
        $this->access_only_allowed_members();
        ...
        return $this->template->view("clients/clients_list_2", $view_data);
    }
    ...
    function serviciosActivos($client_id)
    {
        validate_numeric_value($client_id);
        $this->_validate_client_view_access($client_id);
        ...
        if ($client_id) {
            $view_data["client_info"] = $this->Clients_model->get_one($client_id);
            $view_data["instance_info"] = $this->Api_evolution_instances_model->get_details(["client_id" => $client_id])->getRow();
            $view_data['Medica_item'] = $this->Items_model->get_one($view_data['client_info']->tipo_licencia_medica);
            $view_data['Whatsapp_item'] = $this->Items_model->get_one($view_data['client_info']->tipo_licencia_wpp);
            $view_data['fe_item'] = $this->Items_model->get_one($view_data['client_info']->tipo_licencia_fe);
            $view_data['Contable_item'] = $this->Items_model->get_one($view_data['client_info']->tipo_licencia_contabilidad);
            $view_data['Nomina_item'] = $this->Items_model->get_one($view_data['client_info']->tipo_licencia_nomina);
            $view_data['client_id'] = $client_id;

            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";

            return $this->template->view("clients/serviciosActivos/index", $view_data);
        }
    }
    ...
    public function modal_instance_create_EA($client_id)
    {
        $view_data["client_info"] = new stdClass;
        $view_data["client_info"]->client_id = $client_id;
        return $this->template->view("clients/serviciosActivos/modal_instance_create_EA", $view_data);
    }

    public function modal_instance_connect_EA($client_id)
    {
        $view_data["client_info"] = $this->Clients_model->get_details(["id" => $client_id])->getRow();
        $view_data["instance_info"] = $this->Api_evolution_instances_model->get_details(["client_id" => $client_id])->getRow();
