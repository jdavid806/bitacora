...
class Attendance extends Security_Controller
{
    ...
    function note_modal_form($user_id = 0, $id_attendance = 0, $type = "", $is_break_finish = "")
    {
        $this->validate_submitted_data(array(
            "id" => "numeric|required"
        ));
        ...
        $view_data["info_attendance"] = ["id_attendance" => $id_attendance, "type" => $type, "is_break_finish" => $is_break_finish];
    ...
    //clock in / clock out
    function log_time($user_id = 0, $type = "", $id_attendance = 0)
    {
        validate_numeric_value($user_id);

        $note = $this->request->getPost('note');
        $photo = str_replace("~", ":", $this->request->getPost("photo_data"));
        ...
        if ($id_attendance) {
            $info_attendance = $this->Attendance_model->get_details(["id" => $id_attendance])->getRow();
            delete_app_files(get_setting("timeline_file_path"), array(@unserialize($info_attendance->photo)));
        }

        $photo_data = serialize(move_temp_file("photo_job.png", get_setting("timeline_file_path"), "", $photo));

        $this->Attendance_model->log_time($user_id ? $user_id : $this->login_user->id, $note, $type, $photo_data);
        ...
        if ($user_id > 0) {
            ...
            echo json_encode(array("success" => true, "isUpdate" => false));
        }
    }
    ...
    private function _make_clock_in_out_row($data)
    {
        ...
        if (isset($data->attendance_id)) {
            if (!isset($data->out_break_time) && $data->out_break_time == null) {
                $file = @unserialize($data->photo);
                $img_url = "";
                if ($file) {
                ...
                // El usuario ha registrado la entrada pero no la salida a descanso
                ...
                $view_data = modal_anchor(
                    get_uri("attendance/note_modal_form/$data->id/$data->attendance_id/" . "out_break_time"),
                    ...
                    array(
                        ...
                        "data-post-id" => $data->attendance_id,
                        "data-post-clock_out" => 1,
                        "data-post-id" => $data->id
                    )
                );
            } elseif (!isset($data->in_break_time) && $data->in_break_time == null) {
                // El usuario ha salido a descanso pero aún no ha regresado
                ...
                $out_break_time = format_to_time($data->out_break_time);
                $out_break_datetime = format_to_datetime($data->out_break_time);
                $status = "<div class='mb15' title='$out_break_datetime'>" . app_lang('clock_started_at') . " : $out_break_time</div>";
                $view_data = modal_anchor(
                    get_uri("attendance/note_modal_form/$data->id/$data->attendance_id/" . "in_break_time"),
                ...
                $status = "<div class='mb15' title='$in_datetime'>" . app_lang('clock_started_at') . " : $in_time</div>";
                $view_data = modal_anchor(
                    get_uri("attendance/note_modal_form/$data->id/$data->attendance_id/" . "clock_out/1"),
            ...
            $view_data = modal_anchor(
                get_uri("attendance/note_modal_form/$data->id/0/" . "clock_in"),
