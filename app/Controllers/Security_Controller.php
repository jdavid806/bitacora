...
class Security_Controller extends App_Controller
{
    ...
    protected function make_items_dropdown($type = "", $items_ids = "", $is_filter = false, $custom_filter_title = "")
    {
        if (!$type) {
            show_404();
        }

        $items_dropdown = $is_filter ? array(array("id" => "", "text" => "- " . ($custom_filter_title ? $custom_filter_title : app_lang("label")) . " -")) : array();
        ...
        if ($items_ids) {
            $add_item_option = true;

            //check if any string is exists, 
            //if so, not include this parameter
            $explode_ids = explode(',', $items_ids);
            foreach ($explode_ids as $item_id) {
                if (!is_int($item_id)) {
                    $add_item_option = false;
                    break;
                }
            }

            if ($add_item_option) {
                $options["items_ids"] = $items_ids; //to edit labels where have access of others
            }
        }

        $items = $this->Items_model->get_details($options)->getResult();
