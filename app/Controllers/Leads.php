...
class Leads extends Security_Controller
{
    ...
    function discarded()
    {
        ...
        return $this->template->rander("leads/discarded", $view_data);
    }
    ...
    private function make_lead_modal_form_data($lead_id = 0)
    {
        ...
        $view_data['countries'] = $this->Paises_model->get_details()->getResult();
        $view_data['especialidades'] = $this->Especialidades_model->get_details()->getResult();
        ...
        $view_data['item_suggestions'] = $this->make_items_dropdown("client", $view_data['model_info']->productos_interes);


        $view_data['interest_products_info'] = array(
            array("id" => "1", "text" => "medicalsoft"),
            array("id" => "2", "text" => "nominasoft"),
            array("id" => "3", "text" => "contablesoft"),
        );
    ...
    function save()
    {
        ...
        $interest_products = $this->request->getPost('interest_products');
        validate_list_of_numbers($labels);
        ...
        $data = array(
            ...
            "productos_interes" => $interest_products,
    ...
    function list_data()
    {
        ...
        foreach ($list_data as $data) {
            ...
            $notes_result = $this->Notes_model->get_details(['client_id' => $data->id])->getResult();
            $data->list_notes = $notes_result;
    ...
    function list_data_only_discarded()
    {
        $this->access_only_allowed_members();
        $custom_fields = $this->Custom_fields_model->get_available_fields_for_table("leads", $this->login_user->is_admin, $this->login_user->user_type);
    ...
    private function _make_row($data, $custom_fields)
    {
        ...
        $notes_content = '<ul>';
        if (!empty($data->list_notes)) {
            foreach ($data->list_notes as $note) {
                $notes_content .= '<li>' . $note->title . '</li>';
            }
        }
        $notes_content .= '</ul>';
        ...
        $row_data[] = js_anchor(
            $data->lead_status_title,
            array(
                "style" => "background-color: $data->lead_status_color",
                "class" => "badge",
                "data-id" => $data->id,
    ...
    function view($client_id = 0, $tab = "")
    {
        ...
        if ($client_id) {
            ...
            if ($lead_info && $lead_info->is_lead) {
                ...
                $view_data["is_discard"] = $this->Clients_model->get_is_discard($client_id);
    ...
    function company_info_tab($client_id = 0)
    {
        if ($client_id) {
            ...
            // return $this->template->view('leads/contacts/company_info_tabVista', $view_data);
        }
    }

    function company_info_tabError($client_id = 0)
    {
        if ($client_id) {
            validate_numeric_value($client_id);
            $this->validate_lead_access($client_id);
    ...
    function messages($client_id)
    {
        ...
        return $this->template->view("leads/messages/index", $view_data);
    }

    function send_messages($client_id)
    {
        validate_numeric_value($client_id);
        $this->validate_lead_access($client_id);
        ...
        $view_data['client_id'] = clean_data($client_id);
        ///
        return $this->template->view("leads/messages/table", $view_data);
    }

    function make_calls($client_id)
    {
        validate_numeric_value($client_id);
        $this->validate_lead_access($client_id);
        ...
        ///
        return $this->template->view("leads/calls/index", $view_data);
    }

    function message_template_variables($template_id)
    {
        validate_numeric_value($template_id);

        $template = $this->Wb_Templates_model->get_one($template_id);

        $view_data['template'] = $template;
        ///
        return $this->template->view("leads/messages/variables", $view_data);
    }

    function message_template_preview($template_id)
    {
        validate_numeric_value($template_id);
        ...
        $view_data['template'] = $template;
        ///
        return $this->template->view("leads/messages/preview", $view_data);
    }
    ...
    private function _get_headers_for_import()
    {
        return array(
            ...
            array(
                "name" => "type",
                "required" => true,
                "required_message" => app_lang("import_error_type_field_required"),
                "custom_validation" => function ($type, $row_data) {
            ...
            array(
                ...
                "custom_validation" => function ($contact_last_name, $row_data) {
