...
class Notes extends Security_Controller {
    ...
    function modal_form() {
        ...
        $view_data['titulos'] = array(
            "No contesta" => "No contesta",
            "No contesta - Se envia Whatsapp" => "No contesta - Se envia Whatsapp",
            "Volver a llamar" => "Volver a llamar",
            "Agendo Demostración" => "Agendo Demostración",            
            "Demostración Realizada" => "Demostración Realizada",            
            "Reagendó Demostración" => "Reagendó Demostración",
            "Cotización Enviada" => "Cotización Enviada",
            "Evaluando Cotización" => "Evaluando Cotización",            
            "Solicita una segunda demostración" => "Solicita una segunda demostración",
            "Correo de seguimiento" => "Correo de seguimiento",
            "Pendiente de aprobación interna" => "Pendiente de aprobación interna",
            "Evaluando otras opciones" => "Evaluando otras opciones",
            "Interesado, pero no es el momento adecuado" => "Interesado, pero no es el momento adecuado",
            "Solicito Descuento" => "Solicito Descuento",
            "Pidió referencias de otros clientes" => "Pidió referencias de otros clientes",
            "Esperando decisión final Interna" => "Esperando decisión final Interna",
            "Pidió llamar en unas semanas" => "Pidió llamar en unas semanas",            
        );
    ...
    function modal_form_discard($lead_id = 0) {
        $this->can_access_notes();
        ...
        $view_data['model_info'] = $this->Notes_model->get_one($this->request->getPost('id'));
        $view_data['project_id'] = $this->request->getPost('project_id') ? $this->request->getPost('project_id') : $view_data['model_info']->project_id;
        $view_data['client_id'] = $this->request->getPost('client_id') ? $this->request->getPost('client_id') : $lead_id;
        $view_data['user_id'] = $this->request->getPost('user_id') ? $this->request->getPost('user_id') : $view_data['model_info']->user_id;
        
        $view_data['titulos'] = array(
            "Presupuesto insuficiente" => "Presupuesto insuficiente",
            "Costo del sistema" => "Costo del sistema",
            "Incompatibilidad tecnológica" => "Incompatibilidad tecnológica",
            "Falta de autoridad de decisión" => "Falta de autoridad de decisión",
            "Necesidades no alineadas" => "Necesidades no alineadas",
            "Preferencia por la competencia" => "Preferencia por la competencia",
        ...
        return $this->template->view('notes/modal_form_discard', $view_data);
    }
    ...
    function save_discard() {
        ...
        $this->Clients_model->set_descarted($client_id, "descartado", 1);
        $this->Clients_model->set_descarted($client_id, "descarted_by", $this->login_user->id);
