...
class Announcements extends Security_Controller {
    ...
    function index() {
        ...
        return $this->template->rander("segmentation/index", $view_data);
    ...
    function form($id = 0) {
        ...
        $view_data['model_info'] = $this->Segmentation_model->get_one($id);
        ...
        return $this->template->rander('segmentation/modal_form', $view_data);
    ...
    function view($id = 0) {
        ...
        if ($id) {
            ...
            $announcement = $this->Segmentation_model->get_details($options)->getRow();
            ...
            if ($announcement) {
                ...
                $this->Segmentation_model->mark_as_read($id, $this->login_user->id);

                return $this->template->rander("segmentation/view", $view_data);
    ...
    function save() {
        ...
        if ($id) {
            ...
            $expense_info = $this->Segmentation_model->get_one($id);
        ...
        $save_id = $this->Segmentation_model->ci_save($data, $id);
        ...
        if ($save_id) {
            ...
            echo json_encode(array("success" => true, "recirect_to" => get_uri("segmentation/form/" . $save_id), 'id' => $save_id, 'message' => app_lang('record_saved')));
    ...
    function download_announcement_files($id = 0) {
        ...
        $info = $this->Segmentation_model->get_details($options)->getRow();
    ...
    function delete() {
        ...
        if ($this->request->getPost('undo')) {
            ...
            if ($this->Segmentation_model->delete($id, true)) {
            ...
            if ($this->Segmentation_model->delete($id)) {
    ...
    function list_data() {
        ...
        //show only the allowed segmentation

        $options = $this->_prepare_access_options();



        $list_data = $this->Segmentation_model->get_details($options)->getResult();
    ...
    private function _row_data($id) {
        ...
        $data = $this->Segmentation_model->get_details($options)->getRow();
    ...
    private function _make_row($data) {
        ...
        if ($this->access_type === "all") {
            ...
            $option = anchor(get_uri("segmentation/form/" . $data->id), "<i data-feather='edit' class='icon-16'></i>", array("class" => "edit", "title" => app_lang('edit_announcement')))

                . js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('delete_announcement'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("segmentation/delete"), "data-action" => "delete"));

        }

        return array(

            anchor(get_uri("segmentation/view/" . $data->id), $data->title, array("class" => "", "title" => app_lang('view'))),
