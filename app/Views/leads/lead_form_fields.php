    ...
    <div class="form-group">
        <div class="row">
            <?php if ($model_info->type == "person") { ?>
                <label for="name"
                    class="<?php echo $label_column; ?> company_name_section"><?php echo app_lang('name'); ?></label>
            <?php } else { ?>
                <label for="company_name"
                    class="<?php echo $label_column; ?> company_name_section"><?php echo app_lang('company_name'); ?></label>
            <?php } ?>
            <div class="<?php echo $field_column; ?>">
                <?php
...
<div class="form-group">
    <div class="row">
        <label for="owner_id" class="<?php echo $label_column; ?>"><?php echo app_lang('owner'); ?>
            <span class="help" data-container="body" data-bs-toggle="tooltip"
                title="<?php echo app_lang('the_person_who_will_manage_this_lead') ?>"><i data-feather="help-circle"
    ...
    <div class="row">
        ...
        <div class="<?php echo $field_column; ?>">
            ...
            foreach ($countries as $country) {
                $lead_country[$country->nombre] = $country->nombre;
            }

            echo form_dropdown("country", $lead_country, array($model_info->country), "class='select2'");
            ?>
        </div>
    </div>
</div>
   ...
   <div class="<?php echo $field_column; ?>">
       <select name="country" class='select2'>
           <?php foreach ($countries as $country): ?>
               <option value="<?php echo $country->nombre; ?>"><?php echo $country->nombre; ?></option>
           <?php endforeach; ?>
       </select>

       <?php
       // Inicializamos el array para el dropdown
       // $countries_dropdown = array();

       // Recorremos el array de países y asignamos id => name al array del dropdown
       // foreach ($countriesPrueba as $countryXD) {
       //     echo $countryXD ['nombre'];
       // }

       // // Creamos el form_dropdown con la lista de países
       // echo form_dropdown("country", $countries, array($model_info->countries_id), "class='select2'");

       // O si prefieres un input manual, descomenta lo siguiente
       echo form_input(array(
           "id" => "country",
           "name" => "country",
    ...
    <div class="form-group">
        <div class="row">
            <label for="currency_symbol"
                class="<?php echo $label_column; ?>"><?php echo app_lang('currency_symbol'); ?></label>
            <div class="<?php echo $field_column; ?>">
                <?php
                echo form_input(array(
                    "id" => "currency_symbol",
...
<div class="form-group">
    <div class="row">
        <label for="especialidad" class="<?php echo $label_column; ?>"><?php echo app_lang('specialty') ?></label>
        <div class="<?php echo $field_column; ?>">
            <?php
            $lead_especiality = array();

            foreach ($especialidades as $source) {
                $lead_especiality[$source->nombre] = $source->nombre;
            }

            echo form_dropdown("especialidad", $lead_especiality, array($model_info->especialidad), "class='select2'");
            ?>
        </div>
    </div>
</div>
...
<div class="form-group">
    <div class="row">
        <label for="interest_products"
            class="<?php echo $label_column; ?>"><?php echo app_lang('interest_products') ?></label>
        <div class="<?php echo $field_column; ?>">
            <?php
            echo form_input(array(
                "id" => "interest_products",
                "name" => "interest_products",
                "value" => $model_info->productos_interes,
                "class" => "form-control",
                "placeholder" => app_lang('interest_products')
            ));
            ?>
        </div>
    </div>
...
<div class="form-group">
    <div class="row">
        <label for="comentarios" class="<?php echo $label_column; ?>"><?php echo app_lang('comments') ?></label>
        <div class="<?php echo $field_column; ?>">
            <?php
            echo form_textarea(array(
                "id" => "comentarios",
                "name" => "comentarios",
                "value" => $model_info->comentarios ? $model_info->comentarios : "",
                "class" => "form-control",
                "placeholder" => app_lang('comments')
            ));
            ?>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $("#interest_products").select2({
            multiple: true,
            data: <?php echo json_encode($item_suggestions); ?>
        });
