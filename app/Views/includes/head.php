...
*/
    ...
    <!-- <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var existingTextareas = document.querySelectorAll('textarea[data-rich-text-editor]');

            existingTextareas.forEach(applySCEditor);

            var observer = new MutationObserver(function(mutationsList) {
                mutationsList.forEach(function(mutation) {
                    if (mutation.type === 'childList') {
                        mutation.addedNodes.forEach(function(node) {
                            if (node.tagName && node.tagName.toLowerCase() === 'textarea' && node.hasAttribute('data-rich-text-editor')) {
                                applySCEditor(node);
                            }

                            if (node.querySelectorAll) {
                                var newTextareas = node.querySelectorAll('textarea[data-rich-text-editor]');
                                newTextareas.forEach(applySCEditor);
                            }
                        });
                    }
                });
