    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <?php if ($model_info->description) { ?>
                <div class="col-md-12">
                    <blockquote class="font-14 text-justify" style="<?php echo "border-color:" . $model_info->color; ?>">
            ...
            <?php if ($model_info->location) { ?>
                <div class="col-md-12 mt5">
                    <div class="font-14"><i data-feather="map-pin" class="icon-16"></i>
                        <?php echo custom_nl2br($model_info->location); ?></div>
                </div>
            <?php }
            ?>
                    ...
                    <div class="pl10 pr10">
                        <div class="row">
                            <div class="col-md-1 p0">
                                <span title="<?php echo app_lang("confirmed"); ?>" class='confirmed-by-logo'><span
                                        data-feather="check-circle"></span></span>
                            </div>
            ...
            <?php if ($rejected_by) { ?>
                <div class="col-md-12 clearfix">
                    <div class="pl10 pr10">
                        <div class="row">
                            <div class="col-md-1 p0">
                                <span title="<?php echo app_lang("rejected"); ?>" class="rejected-by-logo"><i
...
<div class="modal-footer">
    <?php echo js_anchor(
        "<i data-feather='" . ($model_info->attended_demo == 1 ? 'x-circle' : 'check-circle') . "' class='icon-16'></i> " . ($model_info->attended_demo == 1 ? app_lang('mark_as_not_attending') : app_lang('mark_as_attending')),
        array(
            "class" => "btn float-start" . ($model_info->attended_demo == 1 ? " btn-danger" : " btn-success"),
            "id" => "toggle_attendance",
    ...
    ?>
    <button type="button" class="btn btn-info text-white close-modal" data-bs-dismiss="modal"><span data-feather="x"
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#toggle_attendance').click(function () {
            ...
            $.ajax({
                url: "<?php echo get_uri('events/toggle_event_attended_status') ?>",
                type: 'POST',
                dataType: 'json',
                data: { encrypted_event_id: encrypted_event_id },
                success: function (result) {
                    if (result.success) {
                        ...
                        console.log(result.attended_status);

                        if (result.attended_status == 1) {
                            $('#toggle_attendance').html("<i data-feather='x-circle' class='icon-16'></i> <?php echo app_lang('mark_as_not_attending'); ?>");
                            $('#toggle_attendance').addClass("btn-danger").removeClass("btn-success");
                        } else if (result.attended_status == 0) {
                            $('#toggle_attendance').html("<i data-feather='check-circle' class='icon-16'></i> <?php echo app_lang('mark_as_attending'); ?>");
                            $('#toggle_attendance').addClass("btn-success").removeClass("btn-danger");
                        }
                    } else {
